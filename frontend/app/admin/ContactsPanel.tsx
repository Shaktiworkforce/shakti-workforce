              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((s) => (
                  <tr key={s.id} className="hover:bg-amber-50/30 transition-colors">
                    <td className="px-5 py-4"><div className="font-semibold text-[#2563eb] text-sm">{s.name}</div></td>
              ...
              {filtered.map((s) => (
                <div key={s.id} className="p-4">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div><div className="font-semibold text-[#2563eb]">{s.name}</div><div className="text-xs text-gray-400 mt-0.5">{formatDate(s.created_at)}</div></div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-4 mb-4 flex flex-col sm:flex-row gap-3 sm:items-center">
      ...
      <div className="flex gap-1.5 bg-gray-100 rounded-lg p-1">
        {filters.map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-md text-xs font-semibold capitalize transition-colors ${filter === f ? 'bg-white text-[#2563eb] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{f}</button>
        ))}
      </div>
    </div>
  );
...
export function EmptyState({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center px-4">
      ...
      <h3 className="font-bold text-[#2563eb] mb-1">Nothing here yet</h3>
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl animate-slide-in-left">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="font-bold text-[#2563eb]">Submission Details</h2>
        ...
        <div className="p-6 space-y-5">
          <div>
            ...
            <div className="text-lg font-bold text-[#2563eb]">{item.name}</div>
          </div>
          <div className="grid grid-cols-1 gap-3">
            <a href={`mailto:${item.email}`} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-amber-50 transition-colors"><Mail size={18} className="text-amber-600" /><div><div className="text-xs text-gray-400">Email</div><div className="text-sm text-[#2563eb] font-medium break-all">{item.email}</div></div></a>
            <a href={`tel:${item.phone}`} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-amber-50 transition-colors"><Phone size={18} className="text-amber-600" /><div><div className="text-xs text-gray-400">Phone</div><div className="text-sm text-[#2563eb] font-medium">{item.phone}</div></div></a>
            <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50"><Calendar size={18} className="text-amber-600" /><div><div className="text-xs text-gray-400">Submitted</div><div className="text-sm text-[#2563eb] font-medium">{formatDate(item.created_at)} at {formatTime(item.created_at)}</div></div></div>
          </div>
          <div>
          ...
          <div className="pt-4 border-t border-gray-100 flex gap-2">
            <a href={`mailto:${item.email}?subject=Re: Your inquiry to Shakti Workforce&body=Dear ${item.name},%0D%0A%0D%0AThank you for contacting us.`} className="flex-1 inline-flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-colors"><Mail size={16} /> Reply via Email</a>
