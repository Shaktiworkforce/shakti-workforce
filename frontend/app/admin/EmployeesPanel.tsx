        ...
        <div className="flex gap-1.5 bg-gray-100 rounded-lg p-1 shrink-0 self-start md:self-auto">
          {(['all', 'new', 'reviewed', 'archived'] as Filter[]).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-md text-xs font-semibold capitalize transition-colors ${filter === f ? 'bg-white text-[#2563eb] shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
            >
              {f}
            </button>
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-amber-50/30 transition-colors">
                      <td className="px-5 py-4">
                        <div className="font-semibold text-[#2563eb] text-sm">{s.full_name}</div>
                ...
                return (
                  <div key={s.id} className="p-4">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div><div className="font-semibold text-[#2563eb]">{s.full_name}</div><div className="text-xs text-gray-400 mt-0.5">{formatDate(s.created_at)}</div></div>
                    ...
                    {loc && (
                      <div className="text-xs text-amber-900 bg-amber-50 flex items-center gap-1.5 p-2 rounded-lg border border-amber-100 mb-2">
                        ...
                        <div>
                          ...
                          <span className="font-semibold text-[#2563eb]">{loc}</span>
                        </div>
                      </div>
                    )}
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl animate-slide-in-left">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <h2 className="font-bold text-[#2563eb]">Application Details</h2>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <div className="text-lg font-bold text-[#2563eb]">{selected.full_name}</div>
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-[#2563eb] font-medium">{getLocation(selected) || 'Not Specified'}</div>
                  </div>
                </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex gap-2">
                <a href={`mailto:${selected.email}?subject=Re: Your application to Shakti Workforce&body=Dear ${selected.full_name},%0D%0A%0D%0AThank you for your application.`} className="flex-1 inline-flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-colors"><Mail size={16} /> Reply</a>
