...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <aside className="hidden md:flex flex-col w-60 bg-[#2563eb] text-white fixed h-screen">
        <div className="p-5 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-sm leading-tight">Shakti Workforce Private Limited</div>
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-20">
          <div className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="min-w-0">
              <h1 className="text-lg font-bold text-[#2563eb]">{current.label}</h1>
          ...
          <div className="hidden md:flex gap-1.5 bg-gray-100 rounded-xl p-1 mb-6 w-fit">
            {tabs.map((t) => (
              <button
                key={t.id}
                onClick={() => setActiveTab(t.id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === t.id ? 'bg-white text-[#2563eb] shadow-sm' : 'text-gray-500 hover:text-gray-700'
                  }`}
              >
      ...
      {isResetOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#2563eb]/60 backdrop-blur-sm" onClick={() => setIsResetOpen(false)} />
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleResetPassword} className="p-6 space-y-4">
              <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl">
                ...
                <div className="text-xs font-bold text-[#2563eb] truncate">{user.email}</div>
              </div>
